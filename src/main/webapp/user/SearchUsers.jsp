    ...
    <div class="row" style="box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1);">
        <div class="col-md-8">
            <c:forEach var="user" items="${searchList}">
                <div style="box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1);margin-block: 10px;" class="user-card d-flex align-items-center p-3 border-bottom">
                    <!-- Avatar -->
                    <div class="avatar">
                        <img src="${pageContext.request.contextPath}/uploads/avatars/${user.image}" alt="Avatar" class="rounded-circle" width="50" height="50">
                    </div>
                    ...
                    <div class="actions">
                        <c:choose>
                            <c:when test="${user.friendStatus == 1}">
                                <a class="btn btn-primary btn-sm">Nhắn tin</a>
                            </c:when>
                            <c:when test="${user.friendStatus == 2}">
                                <a class="btn btn-warning btn-sm">Chấp nhận</a>
                            </c:when>
                            <c:when test="${user.friendStatus == 3}">
                                <a class="btn btn-warning btn-sm">Hủy lời mời</a>
                            </c:when>
                            <c:otherwise>
                                <a class="btn btn-success btn-sm">Thêm bạn bè</a>
                            </c:otherwise>
                        </c:choose>
                    </div>
                </div>
